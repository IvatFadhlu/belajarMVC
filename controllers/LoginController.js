...
//       if ((user, password)) {
//         // Bandingkan password yang diinputkan dengan password yang dienkripsi di database
//         const passwordMatch = await bcrypt.compare(password, user);

//         if (passwordMatch) {
//           // Jika password cocok, set cookie authToken dengan nilai yang unik
//           const authToken = generateAuthToken();
  ...
  static async processLogin(req, res) {
    try {
      ...
      if (user) {
        ...
        const match = await UserModel.comparePassword(user);
